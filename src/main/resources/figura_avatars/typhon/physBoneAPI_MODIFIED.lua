...
-- modified by PirateSee
-- added angle limits by hijacking the render function, probably very buggy but it works
-- physBoneObj:setAngleLimits({pitchmin,pitchmax,yawmin,yawmax})
-- table = physBoneObj:getAngleLimits()
...
function physBone.physBoneRender(delta, context, curPhysBoneID)
	...
	-- crappy pirate clamping
	if curPhysBone.limits then
		pitch = math.clamp(pitch, curPhysBone.limits[1],curPhysBone.limits[2])
		yaw = math.clamp(yaw, curPhysBone.limits[3],curPhysBone.limits[4])
	end
	--host:setActionbar(tostring(pitch) .. " " .. tostring(yaw))
