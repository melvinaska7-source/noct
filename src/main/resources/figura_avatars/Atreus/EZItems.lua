...
---@param extrapart ModelPart
---@param firstpart ModelPart
---@param thirdpartright ModelPart
---@param thirdpartleft ModelPart
...
function items:complexReplace(itemtype,extrapart,firstpart,thirdpartright,thirdpartleft,rightequip,rightunequip,leftequip,leftunequip)
    ...
    if type(thirdpartright) ~= "ModelPart" then
        error("Provided third person right hand part isn't a modelpart",2)
    end
    if type(thirdpartleft) ~= "ModelPart" then
        error("Provided third person left hand part isn't a modelpart",2)
    end
    ...
    events.render:register(
        function()
            ...
            if extrapart then extrapart:setVisible((not (righthold or lefthold)) or isPlaying(rightequip,rightunequip) or isPlaying(leftequip,leftunequip)) end
            thirdpartright:setVisible(righthold or isPlaying(rightequip,rightunequip))
            thirdpartleft:setVisible(lefthold or isPlaying(leftequip,leftunequip))

            local newrighthold = righthold
            if oldrighthold ~= newrighthold then
                thirdpartright:setVisible(oldrighthold)
                if extrapart then extrapart:setVisible(newrighthold) end
            ...
            if oldlefthold ~= newlefthold then
                thirdpartleft:setVisible(oldlefthold)
                if extrapart then extrapart:setVisible(newlefthold) end
...
function items:simpleReplace(itemtype,newparts,extrapart)
    ...
    events.tick:register(
        function()
            extrapart:setVisible(not (getItem(player:getHeldItem(),itemtype) or getItem(player:getHeldItem(true),itemtype)))
        end,
    itemtype.."Simple")
    return self
end
