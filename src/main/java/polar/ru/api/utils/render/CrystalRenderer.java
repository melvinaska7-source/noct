...
public final class CrystalRenderer {
    ...
    public static void render(MatrixStack matrices, BufferBuilder buffer, float x2, float y2, float z2, float size, int color) {
        matrices.push();
        ...
        for (int i2 = 0; i2 < FACES.length; ++i2) {
            int[] face = FACES[i2];
            float brightness = FACE_BRIGHTNESS[i2];
            Vector3f v1 = VERTICES[face[0]];
            Vector3f v2 = VERTICES[face[1]];
            Vector3f v3 = VERTICES[face[2]];
            int shadedColor = CrystalRenderer.applyBrightness(color, brightness);
