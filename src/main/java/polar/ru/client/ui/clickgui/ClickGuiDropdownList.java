...
public class ClickGuiDropdownList extends ClickGuiDropdownSetting {

    private final ListSetting setting;
    private final Map<BooleanSetting, AnimationUtils> anims = new HashMap<>();

    public ClickGuiDropdownList(ListSetting setting) {
        this.setting = setting;
        for (BooleanSetting entry : setting.getSettings()) {
            anims.put(entry, new AnimationUtils(entry.isState() ? 1f : 0f, 8f, Easings.CUBIC_OUT));
        }
        setHeight(22);
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY) {
        ...
        for (BooleanSetting entry : setting.getSettings()) {
            ...
            AnimationUtils anim = anims.get(entry);
            anim.update(entry.isState() ? 1f : 0f);
            ...
            font.draw(matrices, label, cx + (chipWidth - textWidth)/2f, cy + (chipHeight - font.getHeight())/2f, textColor);

            offsetX += chipWidth + gap;
            itemsInRow++;
        }

        float totalRows = (float) Math.ceil(setting.getSettings().stream().filter(BooleanSetting::visible).count() / (double) maxPerRow);
    ...
    public void mouseClicked(double mouseX, double mouseY, int button) {
        ...
        for (BooleanSetting entry : setting.getSettings()) {
            ...
            if (HoveringUtils.isHovered(mouseX, mouseY, chipX + offsetX, chipY + offsetY, chipWidth, chipHeight)) {
                ...
                anims.get(entry).update(entry.isState() ? 1f : 0f);
                return;
            }
            offsetX += chipWidth + gap;
            itemsInRow++;
