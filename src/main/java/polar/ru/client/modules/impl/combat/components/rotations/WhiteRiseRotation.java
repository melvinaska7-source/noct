...
public class WhiteRiseRotation
...
implements QClient {
    ...
    public void reset() {
        ...
        this.shakeTicks = 0;
        boolean bl = this.initialized = WhiteRiseRotation.mc.player != null;
        if (WhiteRiseRotation.mc.player != null) {
            this.lastYaw = WhiteRiseRotation.mc.player.getYaw();
            this.lastPitch = WhiteRiseRotation.mc.player.getPitch();
        } else {
            this.lastYaw = 0.0f;
            this.lastPitch = 0.0f;
        }
    ...
    public void updateRotations(LivingEntity target) {
        float yawLimit = 0.0f;
        double dist;
        if (WhiteRiseRotation.mc.player == null || target == null) {
            return;
        }
        if (!this.initialized) {
            this.lastYaw = WhiteRiseRotation.mc.player.getYaw();
        ...
        boolean close = (dist = WhiteRiseRotation.mc.player.getEyePos().distanceTo(target.getPos())) < 2.8;
        boolean ready = WhiteRiseRotation.mc.player.getAttackCooldownProgress(1.0f) > 0.85f && this.aura.getWhiteRiseTicksToAttack() <= 1;
        ...
        float smooth = MathHelper.clamp((float)this.speedAcceleration, (float)0.0f, (float)(WhiteRiseRotation.mc.player.isGliding() ? 0.45f : 0.35f));
        if (ready) {
            smooth = Math.min(smooth + 0.14f, WhiteRiseRotation.mc.player.isGliding() ? 0.55f : 0.42f);
        }
        if (close && ready) {
        ...
        this.updateJerk(yawDiff, pitchDiff, close);
        this.jerkYawSmooth = MathHelper.lerp((float)0.24f, (float)this.jerkYawSmooth, (float)this.jerkYawTarget);
        this.jerkPitchSmooth = MathHelper.lerp((float)0.18f, (float)this.jerkPitchSmooth, (float)this.jerkPitchTarget);
        float deltaYaw = MathHelper.wrapDegrees((float)(targetYaw - this.lastYaw));
        float deltaPitch = targetPitch - this.lastPitch;
        float f2 = WhiteRiseRotation.mc.player.isGliding() ? 52.0f : (yawLimit = ready ? 36.0f : 28.0f);
        float pitchLimit = WhiteRiseRotation.mc.player.isGliding() ? 22.0f : (ready ? 12.0f : 8.0f);
        deltaYaw = MathHelper.clamp((float)deltaYaw, (float)(-yawLimit), (float)yawLimit);
        deltaPitch = MathHelper.clamp((float)deltaPitch, (float)(-pitchLimit), (float)pitchLimit);
        ...
        float yawSpeed = WhiteRiseRotation.mc.player.isGliding() && target.isGliding() ? 360.0f : 53.2f;
    ...
    private void updateJerk(float yawDiff, float pitchDiff, boolean close) {
        float mul;
        if (this.jerkCd > 0) {
            --this.jerkCd;
            this.jerkYawTarget *= 0.72f;
            this.jerkPitchTarget *= 0.75f;
            return;
        }
        float f2 = mul = close ? 0.7f : 1.1f;
        if (yawDiff > 35.0f && this.rnd.nextFloat() > 0.58f) {
            this.jerkYawTarget = (this.rnd.nextFloat() - 0.5f) * 3.2f * mul;
            this.jerkCd = 6 + this.rnd.nextInt(5);
