...
implements QClient {
    ...
    public void updateRotations(LivingEntity target) {
        double dist;
        boolean close;
        if (SpookyTimeRotation.mc.player == null || target == null) {
            return;
        }
        this.lastYawPos = SpookyTimeRotation.mc.player.getYaw();
        this.lastPitchPos = SpookyTimeRotation.mc.player.getPitch();
        Vec3d eyePos = SpookyTimeRotation.mc.player.getCameraPosVec(1.0f);
        if (target.getBoundingBox().contains(eyePos)) {
            Aura.adjYaw = 0.0f;
            Aura.adjPitch = 0.0f;
            RotationStorage.update(new Rotation(SpookyTimeRotation.mc.player.getYaw(), SpookyTimeRotation.mc.player.getPitch()), 360.0f, 360.0f, 40.0f, 35.0f, 1, 1, Aura.clientLook.isState());
            return;
        }
        ++this.tick;
        Vec3d lookVec = SpookyTimeRotation.mc.player.getRotationVec(1.0f);
        ...
        this.lastTargetYaw = rawTargetYaw;
        this.lastTargetPitch = rawTargetPitch;
        float currentYaw = SpookyTimeRotation.mc.player.getYaw();
        float currentPitch = SpookyTimeRotation.mc.player.getPitch();
        float diffYaw = MathHelper.wrapDegrees((float)(rawTargetYaw - currentYaw));
        float diffPitch = rawTargetPitch - currentPitch;
        diffYaw = MathHelper.clamp((float)diffYaw, (float)-45.0f, (float)45.0f);
        diffPitch = MathHelper.clamp((float)diffPitch, (float)-20.0f, (float)20.0f);
        if (this.postHitTicks > 0) {
            diffYaw *= 0.7f;
            diffPitch *= 0.7f;
        }
    ...
    private void updateJerk(LivingEntity target) {
        ...
        float yawDiff = Math.abs(MathHelper.wrapDegrees((float)(RotationUtils.getRotations((Vec3d)this.stableAim).x - SpookyTimeRotation.mc.player.getYaw())));
        float pitchDiff = Math.abs(RotationUtils.getRotations((Vec3d)this.stableAim).y - SpookyTimeRotation.mc.player.getPitch());
        double dist = SpookyTimeRotation.mc.player.getEyePos().distanceTo(target.getPos());
        ...
        if (yawDiff > 30.0f && this.rnd.nextFloat() > 0.5f) {
            ...
            float sign = Math.signum(MathHelper.wrapDegrees((float)(RotationUtils.getRotations((Vec3d)target.getBoundingBox().getCenter()).x - SpookyTimeRotation.mc.player.getYaw())));
            if (sign == 0.0f) {
