...
public class BooleanComponent extends Component {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        DrawUtil.drawText(Fonts.SFREGULAR.get(), binding ? "Binding..." : setting.getName(),
                x + 4.5f, y + 5f, ColorProvider.setAlpha(ColorProvider.getColorText(), alphaInt), 7.5f, 0.4f, 1f, width - toggleW - 8f);

        float anim = (float) setting.getAnimation().getValue();

        if (anim > 0.01f) {
            // Активный индикатор - colorIndicator БЕЗ градиента
            ...
            DrawUtil.drawRound(toggleX, toggleY, toggleW, toggleH, toggleH / 2f, bgColor);
        } else {
            // Неактивный индикатор - colorInactiveIndicator
            DrawUtil.drawRound(toggleX, toggleY, toggleW, toggleH, toggleH / 2f, ColorProvider.setAlpha(ColorProvider.getColorInactiveIndicator(), alphaInt));
        }
        ...
        // Круглый бегунок внутри овала
        DrawUtil.drawCircle(knobX + knobSize / 2f, knobY + knobSize / 2f, knobSize / 2f, ColorProvider.setAlpha(ColorProvider.getColorSliderCircle(), alphaInt));
