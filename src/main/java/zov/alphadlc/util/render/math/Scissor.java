...
public class Scissor {
    ...
    // Дополнительный масштаб UI (напр. масштаб ClickGui) вокруг центра — чтобы область
    // отсечения совпадала с визуально масштабированным содержимым (шейпы масштабируются
    // через DEFAULT_MATRIX, а не через scissor, поэтому его надо учитывать вручную).
    private static float guiScale = 1f;
    private static float guiCenterX = 0f;
    private static float guiCenterY = 0f;
    ...
    public static void setFromComponentCoordinates(double x, double y, double width, double height) {
        ...
        if (guiScale != 1f) {
            x = guiCenterX + (x - guiCenterX) * guiScale;
            y = guiCenterY + (y - guiCenterY) * guiScale;
            width *= guiScale;
            height *= guiScale;
        }
