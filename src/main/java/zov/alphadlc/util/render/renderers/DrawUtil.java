package zov.alphadlc.util.render.renderers;
...
import zov.alphadlc.util.render.msdf.MsdfRenderer;
import zov.alphadlc.util.render.providers.ResourceProvider;
import zov.alphadlc.util.render.renderers.impl.BuiltBlur;
import zov.alphadlc.util.render.renderers.impl.BuiltRectangle;
...
import static zov.alphadlc.util.render.renderers.IRenderer.DEFAULT_MATRIX;
...
public class DrawUtil implements IMinecraft {
    ...
    // Версия для цветного текста без перезаписи цветов альфой
    public static void drawColoredText(MsdfFont font, Text text, float x, float y, float size) {
        MsdfRenderer.renderText(font, text, size, DEFAULT_MATRIX, x, y + 2, 0);
    }
    ...
    // Гладкий (SDF) круг — как превью цветов в менеджере тем
    public static void drawCircle(float centerX, float centerY, float radius, int color) {
        drawCircleShader(centerX - radius, centerY - radius, radius * 2f, 0f, 0f, (float) (Math.PI * 2.0), color);
    }

    // Гладкая (SDF) кольцевая дуга (углы в градусах, 0° = вправо, по часовой)
    public static void drawRingArc(float centerX, float centerY, float radius, float thickness, float startDeg, float endDeg, int color) {
        drawCircleShader(centerX - radius, centerY - radius, radius * 2f, thickness,
                (float) Math.toRadians(startDeg), (float) Math.toRadians(endDeg), color);
    }

    private static void drawCircleShader(float x, float y, float size, float thickness, float start, float end, int color) {
        if (size <= 0f) return;
        ...
        buffer.vertex(DEFAULT_MATRIX, x, y, 0f).color(color);
        buffer.vertex(DEFAULT_MATRIX, x, y + size, 0f).color(color);
        buffer.vertex(DEFAULT_MATRIX, x + size, y + size, 0f).color(color);
        buffer.vertex(DEFAULT_MATRIX, x + size, y, 0f).color(color);
        BufferRenderer.drawWithGlobalProgram(buffer.end());
